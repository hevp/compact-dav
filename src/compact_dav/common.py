...
class _Logger:
    ...
    class _Formatter(logging.Formatter):
        def format(self, record):
            color = _Logger._COLORS.get(record.levelno, '\x1b[0m')
            label = _Logger._LABELS.get(
                record.levelno,
...
def warning(msg, ret=False):
    _Logger.log(logging.WARNING, msg)
    return ret


def verbose(msg, ret=True):
    if Config.get('verbose', False):
        _Logger.log(logging.INFO, msg, extra={'label': 'verbose'})
    return ret


def debug(msg, force=False, ret=True):
    _Logger.log(logging.INFO if force else logging.DEBUG, msg)
    return ret
